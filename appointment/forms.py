...
class AppointmentForm(forms.ModelForm):
    class Meta:
        model = Appointment
        fields = ['start', 'end', 'patient', 'doctor', 'type', 'description', 'status', 'services']
        widgets = {
            ...
            'type': forms.TextInput(attrs={'class': 'form-control bg-light border-0', 'placeholder': 'Appointment Type', 'style': 'height: 55px;'}),
            'description': forms.Textarea(attrs={'class': 'form-control bg-light border-0', 'placeholder': 'Description', 'style': 'height: 55px;'}),
            'status': forms.TextInput(attrs={'class': 'form-control bg-light border-0', 'placeholder': 'Status', 'style': 'height: 55px;'}),
